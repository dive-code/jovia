...
const WITHDRAWAL_MINIMUMS=Object.freeze({
  regular:30000,
  affiliate:20000
});
...
const MONNIFY_BASE_URL=(
  ...
  (
    String(process.env.MONNIFY_ENV||"sandbox").toLowerCase()==="live"
...
const MONNIFY_ALLOW_UNSIGNED_SANDBOX_WEBHOOKS=
  String(
    process.env.MONNIFY_ALLOW_UNSIGNED_SANDBOX_WEBHOOKS||"true"
  ).toLowerCase()==="true";

const SILVER_WELCOME_BONUS=
  Math.max(0,Number(process.env.SILVER_WELCOME_BONUS||0)||0);

const GOLD_WELCOME_BONUS=
  Math.max(0,Number(process.env.GOLD_WELCOME_BONUS||0)||0);

const UPLOAD_ROOT=
  VOLUME_PATH
    ?path.join(VOLUME_PATH,"uploads")
...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    package TEXT NOT NULL DEFAULT 'Silver'
      CHECK(package IN ('Silver','Gold')),
    package_amount INTEGER NOT NULL DEFAULT 9000,
  ...
  CREATE TABLE IF NOT EXISTS user_job_link_opens (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    job_id INTEGER NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS referral_rewards (
    ...
    referrer_package TEXT NOT NULL,
    referred_package TEXT NOT NULL,
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  CREATE TABLE IF NOT EXISTS withdrawals (
    ...
    user_id INTEGER NOT NULL,
    amount INTEGER NOT NULL,
    type TEXT NOT NULL CHECK(type IN ('regular','affiliate')),
    status TEXT NOT NULL DEFAULT 'pending',
...
function addColumnIfMissing(t,c,d){
  if(!tableExists(t)||columnExists(t,c))return false;
...
function migrateDatabase(){
  ...
  /*
   * Regular withdrawal switch.
   * Default = ON so existing users are not suddenly blocked.
   */
  try{
    const existingSetting=db
      .prepare(
        "SELECT value FROM settings WHERE key='regular_withdrawals_enabled' LIMIT 1"
      )
      .get();
    ...
    if(!existingSetting){
      db.prepare(`
        ...
        VALUES
        ('regular_withdrawals_enabled','1',CURRENT_TIMESTAMP)
      `).run();
    }
  }catch(error){
  ...
  console.log(
    "MIGRATION COMPLETE - LINKS TABLE + REFERRAL_CODE + REFERRALS + SETTINGS"
  );
  ...
  try{
    ...
    if(jobCount===0){
      db.exec(`
        INSERT INTO jobs
        (title,description,category,reward,link,duration,status)
        VALUES
        (
          'Story of life - Watch & Earn',
          'Watch 5 sec to unlock ₦30,000',
          'Videos',
          30000,
          'https://jovianetworkcom.vercel.app/videos.html',
          '5 sec',
          'active'
        ),
        (
          'Daily Check-in',
          'Check in daily to earn ₦500',
          'General',
          500,
          'https://jovianetworkcom.vercel.app',
          'Flexible',
          'active'
        ),
        (
          'Share Jovia on WhatsApp',
  ...
  try{
    ...
    for(const u of users){
      db
        ...
        .run(u.username.toUpperCase(),u.id);
    }
  }catch(e){}
}
...
/* =========================================================
   REGULAR WITHDRAWAL SETTINGS
   ========================================================= */

function isRegularWithdrawalsEnabled(){
  try{
    const row=db
      .prepare(
        "SELECT value FROM settings WHERE key='regular_withdrawals_enabled' LIMIT 1"
    ...
    console.error(
      "WITHDRAWAL SETTING READ ERROR:",
      error
    );

    /*
     * Fail open so a database/settings read problem does not
     * unexpectedly block existing withdrawal functionality.
     */
    return true;
  }
}

function setRegularWithdrawalsEnabled(enabled){
...
function getWelcomeBonus(p){
  return p==="Gold"
    ?GOLD_WELCOME_BONUS
    :SILVER_WELCOME_BONUS;
}
...
function getSessionToken(req,n){
  return parseCookieHeader(
    req.headers.cookie||""
  )[n]||null;
}

function setSessionCookie(res,n,t,m){
  const s=
  ...
  res.setHeader(
    "Set-Cookie",
    `${n}=${t}; HttpOnly; SameSite=None; Path=/; Max-Age=${Math.floor(m/1000)}${s}`
  );
}

function clearSessionCookie(res,n){
  res.setHeader(
    "Set-Cookie",
    `${n}=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0`
  );
}

function createReference(p){
  return(
    p+
    "_"+
    Date.now()+
...
function publicUser(u){
  ...
  return{
    ...
    phone:u.phone,
    package:u.package,
    package_amount:u.package_amount,
    packageAmount:u.package_amount,
    wallet_balance:u.wallet_balance,
    walletBalance:u.wallet_balance,
    affiliate_balance:u.affiliate_balance,
    affiliateBalance:u.affiliate_balance,
...
function paymentResultForUser(p){
  if(!p)return null;
...
function updateUserBalance(id,col,amt){
  if(
    col!=="wallet_balance"&&
    col!=="affiliate_balance"
  ){
...
function recordWalletTransaction(
  ...
  db.prepare(`
    ...
    (user_id,type,amount,balance_type,reference,description)
    VALUES (?,?,?,?,?,?)
  `).run(
    uid,
    type,
...
function activateUserAndRewardsInternal(uid){
  const u=db
    .prepare("SELECT * FROM users WHERE id=?")
    .get(uid);
  ...
  db.prepare(`
    ...
    (referrer_id,referred_user_id,amount,referrer_package,referred_package)
    VALUES (?,?,?,?,?)
  `).run(
    ref.id,
    f.id,
  ...
  if(rew>0){
    ...
    recordWalletTransaction(
      ...
      "affiliate",
      `REFERRAL_${f.id}`,
      `Referral reward for ${f.username} (${f.package})`
    );
  }else{
    db.prepare(`
      UPDATE users
...
function activateUserAndRewards(uid){
  return db.transaction(
    ()=>activateUserAndRewardsInternal(uid)
  )();
}
...
function monnifyRequest(
  ...
  return new Promise((resolve,reject)=>{
    ...
    request.on(
      ...
      ()=>{
        request.destroy(
          new Error("Monnify request timed out.")
        );
      }
    );
...
async function getMonnifyToken(){
  ...
  if(
    ...
    !response.body.requestSuccessful
  ){
    throw new Error(
  ...
  const token=
    response.body.responseBody?.accessToken;
...
async function initializeMonnifyPayment(payment){
  ...
  const response=await monnifyRequest(
    ...
    "/api/v1/merchant/transactions/init-transaction",
    {
      ...
      paymentDescription:
        `Jovia Network ${payment.package} package`,
      currencyCode:"NGN",
  ...
  if(!data.checkoutUrl)
    throw new Error(
      "Monnify returned no checkout URL."
    );

  return data;
}

async function verifyMonnifyPayment(
  paymentReference
){
  const token=await getMonnifyToken();
  ...
  if(
    ...
    throw new Error(
      response.body?.responseMessage||
      "Unable to verify the Monnify transaction."
    );
  }
...
function addWithdrawalEvent(
  ...
  try{
    db.prepare(`
      ...
      (withdrawal_id,action,actor,details)
      VALUES (?,?,?,?)
    `).run(
      withdrawalId,
      action,
    ...
    console.error(
      "WITHDRAWAL EVENT ERROR:",
      error
    );
  }
}

function refundWithdrawalIfNeeded(
  withdrawalId,
  reason,
  actor
){
  return db.transaction(()=>{
    ...
    recordWalletTransaction(
      ...
      row.type==="affiliate"
        ?"affiliate"
        :"wallet",
      `WITHDRAWAL_REFUND_${row.id}`,
      reason||"Withdrawal refunded"
    );
...
function processSuccessfulPayment(
  ...
  return db.transaction(()=>{
    ...
    const paidAmount=Number(
      verificationData?.amountPaid!=null
    ...
    if(
      ...
      Math.round(paidAmount)!==expectedAmount
    ){
      db.prepare(`
        UPDATE payments
        SET
      ...
      throw new Error(
        "Payment amount does not match the expected package amount."
      );
    }

    const transactionReference=
      verificationData?.transactionReference||
      verificationData?.transactionReferenceNumber||
    ...
    activateUserAndRewardsInternal(
      payment.user_id
    );
...
app.get("/api/health",(req,res)=>{
  res.json({
    success:true,
    service:
      "Jovia Network API FIXED + PERSISTENT",
    status:"ok",
    dbPath:DB_PATH,
    volume:VOLUME_PATH||"local",
    uploadRoot:UPLOAD_ROOT,
    videoDir:VIDEO_DIR,
    time:nowIso(),
    regularWithdrawalsEnabled:
      isRegularWithdrawalsEnabled()
  });
});
...
app.post("/api/register",async(req,res)=>{
  try{
    ...
    const welcomeBonus=
      getWelcomeBonus(selectedPackage);
    ...
    try{
      ...
      if(
        ...
        return res.status(400).json({
          success:false,
          message:
            "The selected package does not match your registered package."
        });
      }

      const expectedAmount=
        PACKAGES[req.user.package];
      ...
      let payment=db.prepare(`
        ...
        WHERE user_id=?
        AND amount=?
        AND payment_status IN ('PENDING','INITIATED')
        ORDER BY id DESC
        LIMIT 1
      `).get(
        req.user.id,
      ...
      const redirectUrl=
        ...
        `${req.protocol}://${req.get("host")}/payment-success.html`;
    ...
    try{
      ...
      return res.json({
        ...
        message:
          updatedPayment.payment_status===
          "PAID"
            ?"Payment confirmed and account activated."
            :"Payment has not been confirmed yet.",
        payment:
          paymentResultForUser(
            updatedPayment
          ),
        user:
    ...
    try{
      ...
      try{
        jobs=db.prepare(`
          SELECT
            j.*,
            COALESCE(uj.status,'pending')
              AS user_status,
            CASE
              WHEN ulo.id IS NOT NULL
              THEN 1
              ELSE 0
            END AS link_opened
          FROM jobs j
          LEFT JOIN user_jobs uj
            ON uj.job_id=j.id
            AND uj.user_id=?
          LEFT JOIN user_job_link_opens ulo
            ON ulo.job_id=j.id
            AND ulo.user_id=?
      ...
      return res.json({
        ...
        jobs:jobs.map(job=>({
          ...
          link_opened:
            job.link_opened,
          linkOpened:
            Boolean(job.link_opened),
          completed:
            job.user_status==="completed",
          started:
            job.user_status==="started"
        }))
      });
    ...
    try{
      ...
      if(!opened){
        return res.status(400).json({
          success:false,
          message:
            "Open the task first before claiming."
        });
      }
...
app.get(
  ...
  (req,res)=>{
    try{
      ...
      /*
       * regularWithdrawalsEnabled is exposed here so the
       * member dashboard can immediately disable/enable
       * its regular withdrawal UI.
       *
       * Affiliate withdrawals are intentionally not tied
       * to this setting.
       */
      const regularWithdrawalsEnabled=
        isRegularWithdrawalsEnabled();
      ...
      return res.json({
        ...
        affiliateBalance:
          Number(user.affiliate_balance||0),
        totalEarned:
...
app.post(
  ...
  async(req,res)=>{
    try{
      ...
      /*
       * IMPORTANT:
       * Only REGULAR withdrawals are controlled by the
       * admin ON/OFF switch.
       *
       * Affiliate withdrawals continue to work even when
       * regular withdrawals are OFF.
       */
      if(
        type==="regular"&&
        !isRegularWithdrawalsEnabled()
        ...
        return res.status(403).json({
          success:false,
          message:
            "Regular withdrawals are currently unavailable. Please try again later."
        });
      }

      const amount=
        toPositiveInteger(body.amount);
      ...
      const minimum=
        WITHDRAWAL_MINIMUMS[type];

      if(amount<minimum){
        return res.status(400).json({
          success:false,
          message:
            `Minimum ${type} withdrawal is ₦${minimum.toLocaleString()}.`
        });
      }
...
async function verifyAdminPassword(
  ...
  if(
    configured.startsWith("$2a$")||
    configured.startsWith("$2b$")||
    configured.startsWith("$2y$")
  ){
    ...
    try{
      ...
      const activeUsers=
        db.prepare(`
          ...
          FROM users
          WHERE account_status IN ('active','approved')
        `).get().count;
      ...
      return res.json({
        ...
        /*
         * Main setting for the admin dashboard.
         */
        regularWithdrawalsEnabled,

        /*
         * Also nested here for compatibility with dashboards
         * that read settings through withdrawals.
         */
        withdrawals:{
...
/* =========================================================
   ADMIN REGULAR WITHDRAWAL SWITCH
   ========================================================= */

/*
 * GET current regular withdrawal status.
 */
app.get(
  "/api/admin/settings/regular-withdrawals",
  requireAdmin,
  (req,res)=>{
    try{
      return res.json({
      ...
      return res.status(500).json({
        success:false,
        message:
          "Unable to load regular withdrawal setting."
      });
    }
  }
);
...
/*
 * POST new regular withdrawal status.
 *
 * Accepted:
 * { "enabled": true }
 * { "enabled": false }
  ...
  (req,res)=>{
    try{
      ...
      return res.json({
        ...
        message:
          enabled
            ?"Regular withdrawals are now ON."
            :"Regular withdrawals are now OFF."
      });

    }catch(error){
      console.error(
        "REGULAR WITHDRAWAL SETTING ERROR:",
        error
      );
      ...
      return res.status(500).json({
        success:false,
        message:
          "Unable to update regular withdrawal setting."
      });
    }
  }
);
...
app.get(
  "/api/admin/users/:id/status:statusValue",
  requireAdmin,
  (req,res)=>{
    try{
      const id=
    ...
    try{
      const transferReference=
        cleanString(
          ...
          req.body?.manualTransferReference,
          150
        );
      ...
      const withdrawal=
        db.transaction(()=>{
          ...
          if(row.status!=="pending"){
            throw new Error(
              `Withdrawal is already ${row.status}.`
            );
          }
    ...
    try{
      ...
      if(row.status==="rejected"){
        return res.status(409).json({
          success:false,
          message:
            "This withdrawal is already rejected."
        });
      }

      const refunded=
        refundWithdrawalIfNeeded(
          id,
          reason,
          req.admin.username
        );
    ...
    try{
      ...
      if(
        ...
        return res.status(400).json({
          success:false,
          message:
            "Job reward must be a positive number."
        });
      }
  ...
  (req,res)=>{
    try{
      ...
      if(req.files?.video?.[0]){
        vu=
          `${baseUrl}/uploads/videos/${req.files.video[0].filename}`;
      }

      if(req.files?.thumbnail?.[0]){
        th=
          `${baseUrl}/uploads/videos/${req.files.thumbnail[0].filename}`;
      }
    ...
    try{
      ...
      const result=
        db.transaction(()=>{
          ...
          const rewardRef=
            `VIDEO_REWARD_${req.user.id}_${videoId}`;
...
app.post(
  ...
  async(req,res)=>{
    try{
      ...
      console.log(
        `SELAR ACTIVATION: ${email} => ACTIVE (${plan})`
      );
...
if(require.main===module){
  app.listen(
    ...
    ()=>{
      ...
      console.log(
        " JOVIA NETWORK SERVER - FULLY FIXED + SELAR ACTIVE"
      );
      console.log(
      ...
      console.log(
        ` Volume: ${VOLUME_PATH||"local (add Volume in Railway!)"}`
      );
      console.log(
        ` Admin: http://${HOST}:${PORT}/admin.html`
      );
      console.log(
        " Referral: Gold->Gold 13k, others 8k, Silver->Gold 0"
      );
      console.log(
        ` Regular Withdrawals: ${
          isRegularWithdrawalsEnabled()
