...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, full_name TEXT NOT NULL, username TEXT NOT NULL UNIQUE, email TEXT NOT NULL UNIQUE, phone TEXT NOT NULL, password_hash TEXT NOT NULL, package TEXT NOT NULL DEFAULT 'Silver' CHECK(package IN ('Silver','Gold')), package_amount INTEGER NOT NULL DEFAULT 9000, welcome_bonus INTEGER NOT NULL DEFAULT 0, referrer_id INTEGER, wallet_balance INTEGER NOT NULL DEFAULT 0, affiliate_balance INTEGER NOT NULL DEFAULT 0, total_earned INTEGER NOT NULL DEFAULT 0, account_status TEXT NOT NULL DEFAULT 'pending', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(referrer_id) REFERENCES users(id) ON DELETE SET NULL);
...
function migrateDatabase(){
  ...
  console.log("MIGRATION COMPLETE - LINKS TABLE ADDED");
}
migrateDatabase();
...
function publicUser(u){if(!u)return null;return{id:u.id,full_name:u.full_name,fullName:u.full_name,username:u.username,email:u.email,phone:u.phone,package:u.package,package_amount:u.package_amount,packageAmount:u.package_amount,wallet_balance:u.wallet_balance,walletBalance:u.wallet_balance,affiliate_balance:u.affiliate_balance,affiliateBalance:u.affiliate_balance,total_earned:u.total_earned,totalEarned:u.total_earned,account_status:u.account_status,accountStatus:u.account_status,created_at:u.created_at,createdAt:u.created_at};}
...
// ===== YOUR REQUESTED REFERRAL STRUCTURE - FIXED =====
...
app.get("/api/health",(req,res)=>{res.json({success:true,service:"Jovia Network API FIXED",status:"ok",monnifyConfigured:monnifyConfigured(),time:nowIso()});});
app.post("/api/register",async(req,res)=>{try{const body=req.body||{};const fullName=cleanString(body.fullName??body.full_name,120);const username=cleanString(body.username,50);const email=normalizeEmail(body.email);const phone=cleanString(body.phone,30);const password=String(body.password||"");const selectedPackage=packageFromInput(body.package??body.plan);const referrerId=toPositiveInteger(body.referrerId??body.referrer_id);if(!fullName||!username||!email||!phone||!password||!selectedPackage)return res.status(400).json({success:false,message:"Please complete all required fields."});if(password.length<6)return res.status(400).json({success:false,message:"Password must contain at least 6 characters."});if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return res.status(400).json({success:false,message:"Please enter a valid email address."});const exists=db.prepare(`SELECT id FROM users WHERE lower(username)=lower(?) OR lower(email)=lower(?) LIMIT 1`).get(username,email);if(exists)return res.status(409).json({success:false,message:"That username or email is already registered."});let validReferrerId=null;if(referrerId){const referrer=db.prepare("SELECT id FROM users WHERE id =?").get(referrerId);if(referrer)validReferrerId=referrer.id;}const passwordHash=await bcrypt.hash(password,12);const amount=PACKAGES[selectedPackage];const welcomeBonus=getWelcomeBonus(selectedPackage);const result=db.prepare(`INSERT INTO users (full_name,username,email,phone,password_hash,package,package_amount,welcome_bonus,referrer_id,account_status) VALUES (?,?,?,?,?,?,?,?,?, 'pending')`).run(fullName,username,email,phone,passwordHash,selectedPackage,amount,welcomeBonus,validReferrerId);const userId=Number(result.lastInsertRowid);const token=crypto.randomBytes(32).toString("hex");const maxAge=8*60*60*1000;sessions.set(token,{userId,expiresAt:Date.now()+maxAge});setSessionCookie(res,"jovia_session",token,maxAge);const user=db.prepare("SELECT * FROM users WHERE id =?").get(userId);return res.status(201).json({success:true,message:"Account created successfully.",userId:user.id,package:user.package,amount:user.package_amount,user:publicUser(user)});}catch(error){console.error("REGISTER ERROR:",error);return res.status(500).json({success:false,message:"Unable to create the account."});}});
...
app.get("/api/admin/users",requireAdmin,(req,res)=>{try{const rows=db.prepare(`SELECT id,full_name,username,email,phone,package,package_amount,wallet_balance,affiliate_balance,total_earned,account_status,created_at FROM users ORDER BY id DESC`).all();return res.json({success:true,users:rows.map((user)=>({id:user.id,fullName:user.full_name,full_name:user.full_name,username:user.username,email:user.email,phone:user.phone,package:user.package,packageAmount:Number(user.package_amount||0),walletBalance:Number(user.wallet_balance||0),wallet_balance:user.wallet_balance,affiliateBalance:Number(user.affiliate_balance||0),affiliate_balance:user.affiliate_balance,totalEarned:Number(user.total_earned||0),total_earned:user.total_earned,accountStatus:user.account_status,account_status:user.account_status,createdAt:user.created_at,created_at:user.created_at}))});}catch(error){return res.status(500).json({success:false,message:"Unable to load users."});}});
...
app.get("/api/referrals", requireUser, (req,res)=>{
  try{
    ...
    return res.json({success:true, referrals: refs, rewards});
